function(input, output, session) {
  ...
  output$expert_selectors <- renderUI({
    ...
    lapply(seq_len(n), function(i) {
      selectInput(paste0("expert_model_", i), paste("Expert", i, "model:"),
  ...
  observeEvent(input$elicit_btn, {
    ...
    data_variables   <- input$data_variables %||% ""
    ...
    prompt <- proposal_prompt(clinical_context, data_variables)
  ...
  delphi_data_variables <- reactive({ input$data_variables %||% "" })
  ...
  delphi_server(input, output, session,
                ...
                data_variables_input   = delphi_data_variables)
}
